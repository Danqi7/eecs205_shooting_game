...
BasicBlit PROC USES eax ebx ecx edx edi esi ptrBitmap:PTR EECS205BITMAP , xcenter:DWORD, ycenter:DWORD
 ...
 LOCAL dwWidth : DWORD, dwHeight : DWORD, bTransparent : BYTE,
...
RotateBlit PROC USES eax ebx ecx edx edi esi lpBmp:PTR EECS205BITMAP, xcenter:DWORD, ycenter:DWORD, angle:FXPT
  ...
  shiftX : DWORD, shiftY : DWORD,
  dstWidth : DWORD, dstHeight : DWORD,
...
;; Moves a sprite left
MoveLeft PROC uses ecx sprite:DWORD, velocity:DWORD
	mov ecx, sprite
	mov eax, (SPRITE PTR [ecx]).x
	cmp eax, 0 ;; Make sure I'm not going over the line
	jle AtLeftEdge
  mov eax, velocity
	sub (SPRITE PTR[ecx]).x, eax
AtLeftEdge:
	ret
MoveLeft ENDP
...
MoveRight PROC uses ecx sprite:DWORD, velocity:DWORD
	mov ecx, sprite
	mov eax, (SPRITE PTR [ecx]).x
	cmp eax, 640 ;; Make sure I'm not going over the line
	jge AtRightEdge
  mov eax, velocity
	add (SPRITE PTR[ecx]).x, eax
AtRightEdge:
	ret
MoveRight ENDP

;; 0 if the user is pressing left mouse key, -1 otherwise
